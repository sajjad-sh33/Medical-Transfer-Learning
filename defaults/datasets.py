...
class ISIC2019(BaseSet):
    ...
    def get_data_as_list(self):
        ...
        img_names = [os.path.join(self.root_dir, 'ISIC_2019_Training_Input\ISIC_2019_Training_Input',  imname + '.jpg') for imname in img_names]
...
class APTOS2019(BaseSet):
    ...
    def get_data_as_list(self):
        data_list = []
        datainfo = pd.read_csv(os.path.join(self.root_dir, 'train_1.csv'), engine='python')
        labellist = datainfo.diagnosis.tolist()
        img_names = datainfo.id_code.tolist()
        img_names = [os.path.join(self.root_dir, 'train_images\train_images', imname + '.png') for imname in img_names]
        dataframe = pd.DataFrame(list(zip(img_names, labellist)), 
                                 columns=['img_path', 'label'])
