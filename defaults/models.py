...
from timm.layers import PatchEmbed as embed_layer
...
class Classifier(BaseModel):
    ...
    def __init__(self, model_params):
        ...
        if 'sam2' in self.backbone_type:
            if self.pretrained:
                cpt = download_weights(backbone_type = self.backbone_type, patch_size = 16, pretrained_type = 'sam2')
            ...
            if self.backbone_type == "sam2_b" :
                    model_cfg = "./sam2.1_hiera_b+.yaml"
                    self.backbone = build_sam2(model_cfg, device=device)
                    embed_dim = self.backbone.sam_prompt_encoder.embed_dim
            elif self.backbone_type == "sam2_l" :
                    model_cfg = "./sam2.1_hiera_l.yaml"
                    self.backbone = build_sam2(model_cfg, device=device)
                    embed_dim = self.backbone.sam_prompt_encoder.embed_dim
            self.backbone.load_state_dict(cpt["model"])
            self.backbone = self.backbone.image_encoder
            self.fc = nn.Linear(embed_dim, self.n_classes)
    
            if self.img_channels != 3:
                pretrained_weight = self.backbone.trunk.patch_embed.proj.weight.data
                if self.backbone.trunk.patch_embed.proj.bias is not None:
                    pretrained_bias = self.backbone.trunk.patch_embed.proj.bias.data
                pretrained_weight = pretrained_weight.repeat(1, 4, 1, 1)[:, :self.img_channels]
                self.backbone.trunk.patch_embed.proj = nn.Conv2d(in_channels = self.img_channels, out_channels  = embed_dim, kernel_size=(7, 7), stride=(4, 4),  padding=(3, 3))
    
                self.backbone.trunk.patch_embed.proj.weight.data = pretrained_weight 
                if self.backbone.trunk.patch_embed.proj.bias is not None:
                    self.backbone.trunk.patch_embed.proj.bias.data = pretrained_bias 
            self.embed_dim = embed_dim
            self.blocks = self.backbone.trunk.blocks
        
        elif 'vit_' in self.backbone_type:
            if self.pretrained:
                cpt = download_weights(backbone_type = self.backbone_type, patch_size = 16, pretrained_type = 'mae')
            ...
            if self.img_channels != 3:
                ...
                pretrained_weight = pretrained_weight.repeat(1, 4, 1, 1)[:, :self.img_channels]
                if self.backbone_type == "vit_base_patch16" :
                    self.backbone.patch_embed = embed_layer(img_size = self.img_size, patch_size=16, embed_dim=768, in_chans=self.img_channels)
                elif  self.backbone_type == "vit_large_patch16" :
                    self.backbone.patch_embed = embed_layer(img_size = self.img_size, patch_size=16, embed_dim=1024, in_chans=self.img_channels)
                elif self.backbone_type ==   "vit_huge_patch14" :
                    self.backbone.patch_embed = embed_layer(img_size = self.img_size, patch_size=14, embed_dim=1280, in_chans=self.img_channels)
        ...
        elif 'aimv2' in self.backbone_type:
            if self.backbone_type == "aimv2-large-patch14-224" :
                self.backbone = AutoModel.from_pretrained("apple/aimv2-large-patch14-224", trust_remote_code=True)
                embed_dim = 1024
            elif self.backbone_type == "aimv2-huge-patch14-224" :
                self.backbone = AutoModel.from_pretrained("apple/aimv2-huge-patch14-224", trust_remote_code=True)
                embed_dim = 1536
            
            self.fc = nn.Linear(embed_dim, self.n_classes)

            if self.img_channels != 3:
                pretrained_weight = self.backbone.preprocessor.patchifier.proj.weight.data
                if self.backbone.preprocessor.patchifier.proj.bias is not None:
                    pretrained_bias = self.backbone.preprocessor.patchifier.proj.bias.data
                pretrained_weight = pretrained_weight.repeat(1, 4, 1, 1)[:, :self.img_channels]
                self.backbone.preprocessor.patchifier.proj = nn.Conv2d(in_channels = self.img_channels, out_channels  = embed_dim, kernel_size=14, stride=14)
                
                self.backbone.preprocessor.patchifier.proj.weight.data = pretrained_weight 
                if self.backbone.preprocessor.patchifier.proj.bias is not None:
                    self.backbone.preprocessor.patchifier.proj.bias.data = pretrained_bias 
            self.embed_dim = embed_dim
        elif 'coca' in self.backbone_type:

            if self.backbone_type == "coca_ViT-B-32" :
                self.backbone = open_clip.create_model_and_transforms(model_name="coca_ViT-B-32",pretrained="laion2b_s13b_b90k")[0].visual
                embed_dim = 512
                kernel_size = 32
            if self.backbone_type == "coca_ViT-L-14" :
                self.backbone = open_clip.create_model_and_transforms(model_name="coca_ViT-L-14",pretrained="laion2b_s13b_b90k")[0].visual
                embed_dim = 768
                kernel_size = 14
            ...
            if self.img_channels != 3:
                ...
                self.backbone.conv1 = nn.Conv2d(in_channels = self.img_channels, out_channels  = embed_dim, kernel_size=kernel_size, stride=kernel_size, bias=False)
                
                self.backbone.conv1.weight.data = pretrained_weight 
                if self.backbone.conv1.bias is not None:
                    self.backbone.conv1.bias.data = pretrained_bias 
        elif 'sam' in self.backbone_type:
            if self.pretrained:
    ...
    def forward(self, x, return_embedding=False):
        with autocast(self.use_mixed_precision):
            ...
            if isinstance(x, list) and hasattr(cnn_models, self.backbone_type):
                ...
                if 'vit_' in self.backbone_type:
                    x_emb = self.backbone(x)
                    x_emb = x_emb[:,1:,:]
                    
                    if return_embedding:
                        x_cls = x_emb.mean(dim=1)
                ...
                if 'aimv2' in self.backbone_type:
                    x_emb = self.backbone(x)["last_hidden_state"]
                    if return_embedding:
                        x_cls = x_emb.mean(dim=1)
                        return self.fc(x_cls), x_cls
                    else:
                ...
                if 'coca' in self.backbone_type:
                    x_cls, x_emb = self.backbone(x)
                    if return_embedding:
                        return self.fc(x_cls), x_cls
                    else:
                        if self.linear:
                            ...
                            return torch.cat((x_cls.unsqueeze(1), x_emb), dim=1)

                if 'sam2' in self.backbone_type:
                    x_emb = self.backbone(x)['vision_features']
                    x_emb = x_emb.permute(0, 2, 3, 1)
                    batch_size, b, c, d = x_emb.size()
                    batch_size, height, width, channels = x_emb.size()
                    x_emb = x_emb.reshape(batch_size, height * width, channels)
                    x_cls = torch.mean(x_emb , dim = 1)
                    if return_embedding:
                        return self.fc(x_cls), x_cls
                    else:
                        if self.linear:
...
class Ymodel_sam2(BaseModel):
    def __init__(self, model_params, foundation_model, main_model):
        ...
        self.frozen_foundation = self.foundation_params.freeze_backbone  
        self.embed_dim = main_model.backbone.embed_dim
        self.foundation_embed = foundation_model.embed_dim
...
class Ymodel_coca(BaseModel):
    def __init__(self, model_params, foundation_model, main_model):
        ...
        self.frozen_foundation = self.foundation_params.freeze_backbone  
        self.embed_dim = main_model.backbone.embed_dim
        self.foundation_embed = foundation_model.backbone.output_dim 
