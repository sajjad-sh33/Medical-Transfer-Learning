...
class Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        for self.epoch in epoch_bar:            
            ...
            if self.is_rank0:
                iter_bar = tqdm(iter_bar, desc='Training', leave=False, total=len(self.trainloader), disable=True)
    ...
    def evaluate(self, dataloader=None, **kwargs):
        ...
        if self.is_rank0:
            ...
            iter_bar = tqdm(dataloader, desc='Validating', leave=False, total=len(dataloader), disable=True)
        else:
            iter_bar = dataloader
    ...
    def test(self, dataloader=None, **kwargs):
        ...
        if self.knn_eval or not self.is_supervised:
            knn_metric = self.metric_fn(n_classes, dataloader.dataset.int_to_labels, mode="knn_val")
        iter_bar = tqdm(dataloader, desc='Testing', leave=True, total=len(dataloader), disable=True)
